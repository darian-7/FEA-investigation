function fn_display_result2(nodes, elements, display_options, varargin)
%SUMMARY
...
if ~isempty(display_options.node_sets_to_plot)
    ...
    for ii = 1:length(display_options.node_sets_to_plot)
        plot(nodes(display_options.node_sets_to_plot(ii).nd, 1), nodes(display_options.node_sets_to_plot(ii).nd, 2), display_options.node_sets_to_plot(ii).col,'markersize',10);
    end;
end;
